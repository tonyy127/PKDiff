...
import os
import datetime
import io as pyio               # 标准库
# Parameters
## SwinFusion
# WINDOW_SIZE = (64, 64) # Patch size
WINDOW_SIZE = (256, 256) # Patch size

TEST_STRIDE = 128 # Stride for testing
IN_CHANNELS = 3 # Number of input channels (e.g. RGB)
FOLDER = "/root/autodl-tmp/" # Replace with your "/path/to/the/ISPRS/dataset/folder/"
BATCH_SIZE = 64 # default=10 Number of samples in a mini-batch
...
def metrics1(predictions, gts, label_values=LABELS):
    cm = confusion_matrix(
        gts,
        predictions,
...
def metrics(predictions, gts, label_values=LABELS, log_path="/root/autodl-tmp/logs/metrics.txt", also_print=True):
    """
    计算指标并把“打印内容”同时写入到 log_path（追加模式）。
    - predictions, gts: 一维或可展平数组
    - label_values: 类别名列表（用于逐类指标打印）
    - log_path: 日志文件路径（默认 logs/metrics.txt）
    - also_print: 是否在控制台打印（默认 True）
    返回: mean IoU（与原逻辑一致）
    """
    # —— 日志缓冲区：既打印，也收集到内存里 —— #
    buf = pyio.StringIO()
    ...
    # Global accuracy
    total = cm.sum()
    accuracy = np.trace(cm) * 100.0 / float(total if total > 0 else 1)
    log(f"{total} pixels processed")
    log(f"Total accuracy : {accuracy:.2f}")

    Acc = np.divide(np.diag(cm), cm.sum(axis=1, where=(cm.sum(axis=1)!=0)), out=np.zeros(len(label_values), dtype=float), where=(cm.sum(axis=1)!=0))
    for l_id, score in enumerate(Acc):
        log(f"{label_values[l_id]}: {score:.4f}")
    log("---")

    # F1
    F1Score = np.zeros(len(label_values), dtype=float)
    for i in range(len(label_values)):
        denom = (np.sum(cm[i, :]) + np.sum(cm[:, i]))
        if denom > 0:
            F1Score[i] = 2.0 * cm[i, i] / denom
    log("F1Score :")
    for l_id, score in enumerate(F1Score):
        log(f"{label_values[l_id]}: {score:.4f}")
    log(f"mean F1Score: {np.nanmean(F1Score[:5]):.4f}")
    log("---")

    # Kappa
    total = cm.sum()
    pa = np.trace(cm) / float(total if total > 0 else 1)
    pe = (np.sum(cm.sum(axis=0) * cm.sum(axis=1))) / float(total * total if total > 0 else 1)
    kappa = (pa - pe) / (1 - pe + 1e-12)
    log(f"Kappa: {kappa:.4f}")

    # mIoU
    denom_iou = (np.sum(cm, axis=1) + np.sum(cm, axis=0) - np.diag(cm))
    MIoU_per_class = np.divide(np.diag(cm), denom_iou, out=np.zeros_like(denom_iou, dtype=float), where=(denom_iou!=0))
    log(MIoU_per_class)
    MIoU = np.nanmean(MIoU_per_class[:5])
    log(f"mean MIoU: {MIoU:.4f}")
    log("---")
    ...
    if _is_main_process():
        try:
            ...
            with open(log_path, "a", encoding="utf-8") as f:
                ...
                f.write(f"[{ts}] metrics call\n")
                f.write(buf.getvalue())
                f.write("\n")
        except Exception as e:
            if also_print:
                print(f"[metrics] 写日志失败：{e}")
